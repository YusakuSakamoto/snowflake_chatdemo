...
**案件明細データをそのまま受け入れるための取込（Raw / Landing）テーブル**である。

本テーブルは、業務的に正規化された案件明細の正本を表すものではなく、  
後段で行われる整形・検証・正規化処理のための **一次受け皿**として位置づけられる。
...
そのため、本テーブルは **データ品質を保証する責務を持たない**。

### 制約を設けないという設計判断
[[APP_PRODUCTION.ANKEN_MEISAI]] では、以下を **意図的に行わない**。
