...
## 概要
APP_PRODUCTION.[[design.ANKEN_MEISAI]] は、外部システムやファイル（主に CSV）から提供される  
...
## 設計上の位置づけ
APP_PRODUCTION.[[design.ANKEN_MEISAI]] は、以下のレイヤ構成の最下流（取込側）に位置する。

- 取込（Raw / Landing）  
  → APP_PRODUCTION.[[design.ANKEN_MEISAI]] 
- 整形（型変換・正規化・制約付与）
- 業務・分析用正本（参照・集計）
...
### 制約を設けないという設計判断
APP_PRODUCTION.[[design.ANKEN_MEISAI]] では、以下を意図的に行わない。
...
APP_PRODUCTION.[[design.ANKEN_MEISAI]] は、
