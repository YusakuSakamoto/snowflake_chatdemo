...
## 概要
APP_PRODUCTION.[[design.DEPARTMENT_MASTER]] は、外部システムや CSV ファイルから提供される  
部署マスタ情報を年度単位で取り込むための取込（Raw / Landing）テーブルである。

本テーブルは「部署マスタ」という名称を持つが、  
業務上の正本マスタや参照用マスタを直接表すものではなく、  
後段で整形・正規化された部署マスタを構築するための 一次受け皿として位置づけられる。
...
## 設計上の位置づけ
APP_PRODUCTION.[[design.DEPARTMENT_MASTER]] は、以下のレイヤ構成における取込層に属する。

- 取込（Raw / Landing）  
  → APP_PRODUCTION.[[design.DEPARTMENT_MASTER]]
...
### 年度別マスタとしての扱い
- 本テーブルは 年度（`FISCAL_YEAR`）を含む部署定義のスナップショットを保持する。
...
### 制約を設けないという判断
APP_PRODUCTION.[[design.DEPARTMENT_MASTER]] では、以下を 意図的に行わない。
...
これらは 取込元が定義する粒度・構造をそのまま保持することを目的とする。  
...
APP_PRODUCTION.[[design.DEPARTMENT_MASTER]] は  
