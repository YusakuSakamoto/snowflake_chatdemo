# ビュー設計：V_PROJECT_MASTER

## 概要
APP_PRODUCTION.V_PROJECT_MASTER は、取込テーブル（案件明細）から **案件マスタ相当の情報**を抽出し、参照・分析に適した粒度で提供するための正規化 VIEW である。

取込テーブルは「来たものを保持する層」であり、欠損・重複・形式揺れを許容する。  
本 VIEW はその入力を前提に、分析で扱いやすいよう **粒度の固定**と **代表値の選択**、および **日付の型変換**を行う。
...
その場合、本 VIEW は案件単位に集約するため、名称や分類などの属性について **代表値**を選択する必要がある。
...
本 VIEW では分析・フィルタ・期間計算の利便性を高めるため、作業開始日・終了日について **日付型への変換**を行う。
