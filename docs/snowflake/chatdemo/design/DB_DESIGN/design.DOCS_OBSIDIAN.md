...
### DOCS_OBSIDIAN_V との関係
[[DB_DESIGN.DOCS_OBSIDIAN_V]] は DOCS_OBSIDIAN を基底とする参照専用ビューであり、検索・閲覧・Agent 利用に適した形で情報を提供する層である。
...
DOCS_OBSIDIAN_V は DOCS_OBSIDIAN の内容を変更せず、参照用途のための投影・整形のみを行う。  
Evidence の根拠単位は常に [[DB_DESIGN.DOCS_OBSIDIAN.PATH]] によって示される実在する .md ファイルであり、ビュー側でその意味を変えない。

### DOCS_OBSIDIAN_V から OBSIDIAN_VAULT_SEARCH への連携
...
### INGEST_VAULT_MD プロシージャーとの関係
[[DB_DESIGN.INGEST_VAULT_MD]] は、[[DB_DESIGN.OBSIDIAN_VAULT_STAGE]] に同期された Vault の Markdown を読み取り、DOCS_OBSIDIAN に 1ファイル = 1レコードで UPSERT する唯一の書き込み経路である。
...
### 主キーと識別
- [[DB_DESIGN.DOCS_OBSIDIAN.DOC_ID]] は DOCS_OBSIDIAN における論理的な主キーであり、1ファイルを一意に識別する。
