...
## 概要
DB_DESIGN.[[design.PROFILE_RESULTS]] は、データベーステーブルの各カラムに対して算出されたプロファイル計測結果をS3上に保持し、Snowflakeから外部テーブルとして参照するテーブルである。  
...
- このテーブルが表す概念  
  ...
  - 計測対象は  
    [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_DB]] /  
    [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_SCHEMA]] /  
    [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_TABLE]] /  
    [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_COLUMN]]  
    により特定される。
- 主な利用シーン  
...
DB_DESIGN.[[design.PROFILE_RUNS]] はプロファイル実行単位（run）の管理テーブルであり、  
[[design.PROFILE_RESULTS]] はその run に紐づく結果詳細を保持する。

- 関係性  
  DB_DESIGN.[[design.PROFILE_RUNS]]（1） → DB_DESIGN.[[design.PROFILE_RESULTS]]（N）
- 関連キー  
  - [[DB_DESIGN.[[design.PROFILE_RESULTS]].RUN_ID]]  
    → [[DB_DESIGN.[[design.PROFILE_RUNS]].RUN_ID]]

[[design.PROFILE_RESULTS]] は、必ず既存の run に紐づく結果としてのみ存在する前提とする。

### [[design.PROFILE_TABLE]] との関係
DB_DESIGN.[[design.PROFILE_TABLE]] は、単一テーブルを対象としてプロファイル処理を実行するためのストアドプロシージャである。
...
DB_DESIGN.[[design.PROFILE_COLUMN]] は、単一カラムを対象に即時計測を行うためのプロシージャである。

- [[design.PROFILE_COLUMN]] は参照・計算用途を主目的とし、  
  原則として [[design.PROFILE_RESULTS]] への永続書き込みは行わない。
- ただし、設計上は [[design.PROFILE_COLUMN]] で算出されるメトリクスは  
  [[design.PROFILE_RESULTS]].METRICS と同一構造・同一意味を持つことを前提とする。
- これにより、単発計測結果と永続結果を同一の解釈軸で扱える。

### [[design.PROFILE_ALL_TABLES]] との関係
DB_DESIGN.[[design.PROFILE_ALL_TABLES]] は、複数テーブル（スキーマ単位、DB単位など）を横断してプロファイル処理を実行するオーケストレーション用プロシージャである。

- [[design.PROFILE_ALL_TABLES]] は内部で  
  DB_DESIGN.[[design.PROFILE_TABLE]] を複数回呼び出す構成を取る。
- 各テーブルごとに run が生成され、  
...
- [[design.PROFILE_RESULTS]] への INSERT / UPDATE / DELETE は、以下のプロシージャ群からのみ行われる。
  - DB_DESIGN.[[design.PROFILE_TABLE]]
  - （間接的に）DB_DESIGN.[[design.PROFILE_ALL_TABLES]]
...
### メトリクス表現
- [[DB_DESIGN.[[design.PROFILE_RESULTS]].METRICS]] は VARIANT 型とし、  
  行数、NULL 率、distinct 数、最小値・最大値などを柔軟に格納する。
- メトリクス構造は固定せず、将来拡張を許容する。

### 時点の扱い
- [[DB_DESIGN.[[design.PROFILE_RESULTS]].AS_OF_AT]] は、計測対象データの論理時点を示す。
- 実行時刻や状態は run 側（[[design.PROFILE_RUNS]]）で管理する。

## 注意点
- NOT NULL 列  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].RUN_ID]],  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_COLUMN]],  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].AS_OF_AT]],  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].METRICS]],  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_DB]],  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_SCHEMA]],  
  [[DB_DESIGN.[[design.PROFILE_RESULTS]].TARGET_TABLE]]
...
- 関連テーブル：DB_DESIGN.[[design.PROFILE_RUNS]]
- 関連プロシージャ：DB_DESIGN.[[design.PROFILE_TABLE]], DB_DESIGN.[[design.PROFILE_COLUMN]], DB_DESIGN.[[design.PROFILE_ALL_TABLES]]
