# テーブル設計：[[design.PROFILE_RUNS]]

## 概要
DB_DESIGN.[[design.PROFILE_RUNS]] は、データベーステーブルに対して実行したプロファイル処理（例：行数、列統計、欠損率などの算出）の実行履歴を管理するテーブルである。  
...
- このテーブルが表す概念
    ...
    - 各runは `RUN_ID` により識別され、結果テーブル（[[design.PROFILE_RESULTS]]）と論理的に関連付けられる。
- どの業務で使われるか
    - データ品質管理・モニタリング業務
    ...
    - 再実行・運用判断
      ...
      - 最新の成功runの特定
    - プロファイル結果参照の基点
      - [[design.PROFILE_RESULTS]] 等の結果テーブルを参照する際の起点キーとして利用

## データ更新ポリシー
- 本テーブル（[[design.PROFILE_RUNS]]）および関連する結果テーブル（[[design.PROFILE_RESULTS]]）のデータ更新（INSERT / UPDATE / DELETE）は、
  以下のストアドプロシージャからのみ行われることを前提とする。
    - DB_DESIGN.[[design.PROFILE_TABLE]]
- DB_DESIGN.[[design.PROFILE_COLUMN]] は、単一カラムに対するプロファイル結果を即時計算して返却するための参照・計算専用プロシージャであり、
  永続テーブルへの書き込みは行わない。
