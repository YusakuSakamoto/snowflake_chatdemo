...
- DIM_ENTITY_ALIAS は手動辞書を最優先で取り込む（priority 小が優先）

### 自動生成辞書との関係
- 自動生成：[[APP_PRODUCTION.V_ENTITY_ALIAS_AUTO]]（および同等のDEV側）
  - マスタ/正規化VIEWから機械生成できる名称を展開
- DIM_ENTITY_ALIAS は自動生成辞書も取り込み、手動辞書と統合して winner を決定する

### 統合ビューとの関係
- 統合規則は [[APP_PRODUCTION.V_ENTITY_ALIAS_ALL]]（または同等）で表現される
- DIM_ENTITY_ALIAS は V_ENTITY_ALIAS_ALL を materialize したものとして運用する
  - 重複排除（winner 決定）は V_ENTITY_ALIAS_ALL の規則に従う

## カラム定義の思想

### 識別キー（主キー）
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.ALIAS_NORMALIZED]]
  - 正規化関数（NORMALIZE_JA / NORMALIZE_JA_DEPT 等）適用後の文字列
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.ENTITY_TYPE]]
...
### 表記系
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.ALIAS_RAW]]
  - 人間に見せる元表記（入力表記）
  - 必須ではないが、説明可能性のため保持する

### 解決結果
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.ENTITY_ID]]
  - 対象エンティティのID（共通利用のため VARCHAR）
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.ENTITY_NAME]]
  - 正式名称（UI表示・確認用）

### スコアリング
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.CONFIDENCE]]
  - 0.0〜1.0 の信頼度（「正しさ」ではなく、どれくらい強い別名か）
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.PRIORITY]]
  - 小さいほど優先
  - MANUAL は AUTO より常に小さくする（運用ルール）

### 有効/無効
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.IS_ACTIVE]]
  - true のみ検索対象
  - 無効化は手動辞書側で行い、refresh により反映される

### リフレッシュ追跡
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.REFRESH_RUN_ID]]
  - refresh 実行単位の識別子（timestamp や UUID）
  - 監査・トラブルシュート時に「どの refresh で生成されたか」を追える
- [[NAME_RESOLUTION.DIM_ENTITY_ALIAS.REFRESHED_AT]]
  - refresh 実行時刻
...
### refresh の入力
- refresh の入力は V_ENTITY_ALIAS_ALL の winner のみ（重複排除済）とする
- 既存の DIM_ENTITY_ALIAS の内容に依存した増分更新は前提としない
