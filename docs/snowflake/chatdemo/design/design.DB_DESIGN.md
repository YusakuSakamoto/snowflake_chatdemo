...
本基盤では、**Obsidian Vault（Markdown）を DB 設計の正本**とする。
...
- Snowflake 上の `DB_DESIGN` テーブル群は **設計メタ情報の反映・運用用**
...
- DB を直接参照しない
- Vault（md）だけを見る
- 実データ結果も **md として保存されたもののみ**を根拠にする
...
を **無理なく接続するための最小構成**である。

重要なのはツールではなく、

> **設計を「成果物」ではなく  
> 「運用・改善され続ける資産」として扱うこと**
