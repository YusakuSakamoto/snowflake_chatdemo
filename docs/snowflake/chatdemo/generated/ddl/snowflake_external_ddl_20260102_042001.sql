...
CREATE OR REPLACE EXTERNAL TABLE LOG.CORTEX_CONVERSATIONS(
  ...
  DAY NUMBER AS CAST(SPLIT_PART(SPLIT_PART(metadata$filename, '/', 3), '=', 2) AS NUMBER),
  HOUR NUMBER AS CAST(SPLIT_PART(SPLIT_PART(metadata$filename, '/', 4), '=', 2) AS NUMBER),
  MONTH NUMBER AS CAST(SPLIT_PART(SPLIT_PART(metadata$filename, '/', 2), '=', 2) AS NUMBER),
  YEAR NUMBER AS CAST(SPLIT_PART(SPLIT_PART(metadata$filename, '/', 1), '=', 2) AS NUMBER)
)\nPARTITION BY (YEAR, MONTH, DAY, HOUR)
LOCATION=@LOG.CORTEX_CONVERSATIONS_STAGE
...
LOCATION=@LOG.AZFUNCTIONS_STAGE
FILE_FORMAT=(TYPE=JSON)
AUTO_REFRESH=TRUE
REFRESH_ON_CREATE=TRUE;
...
LOCATION=@LOG.SNOWFLAKE_METRICS_STAGE
FILE_FORMAT=(TYPE=JSON)
AUTO_REFRESH=TRUE
REFRESH_ON_CREATE=TRUE;
